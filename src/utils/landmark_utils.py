...
def extract_landmark_vector(results):
    ...
    face = np.array([[res.x, res.y, res.z] for res in getattr(results, 'face_landmarks', None).landmark]).flatten() if getattr(results, 'face_landmarks', None) else np.zeros(468*3)
