...
def main():
    """Main function to train the enhanced LSTM model using existing splits."""
    try:
        ...
        # Charger les données d'entraînement (utilise les splits existants)
        logger.info("Loading training data from existing train split...")
        ...
        # Charger les données de validation (si elles existent)
        val_dir = os.path.join(data_path, 'val')
        X_val_raw, y_val_raw = [], []
        if os.path.exists(val_dir):
            logger.info("Loading validation data from existing val split...")
            X_val_raw, y_val_raw, _ = load_data_efficiently(val_dir, 'val', max_memory_gb=2)
            logger.info(f"Loaded {len(X_val_raw)} validation samples")
            if len(X_val_raw) == 0:
                logger.warning("Validation directory exists but is empty. Using 15% of training data for validation.")
                n_val = int(len(X_train_raw) * 0.15)
                indices = np.random.permutation(len(X_train_raw))
                val_indices = indices[:n_val]
                train_indices = indices[n_val:]
                X_val_raw = [X_train_raw[i] for i in val_indices]
                y_val_raw = [y_train_raw[i] for i in val_indices]
                X_train_raw = [X_train_raw[i] for i in train_indices]
                y_train_raw = [y_train_raw[i] for i in train_indices]
                logger.info(f"Created validation split: {len(X_val_raw)} val, {len(X_train_raw)} train")
        else:
            logger.warning("No validation data found. Using 15% of training data for validation.")
            n_val = int(len(X_train_raw) * 0.15)
            indices = np.random.permutation(len(X_train_raw))
            val_indices = indices[:n_val]
            train_indices = indices[n_val:]
        ...
        checkpoint_path = os.path.join(models_path, 'lsf_recognition_model_correct.keras')
        ...
        history_path = os.path.join(models_path, 'training_history_correct.json')
        ...
        logger.error(f"ERROR in train_model_correct.py: {str(e)}")
        import traceback
        traceback.print_exc()
        raise
