...
def main():
    ...
    try:
        ...
        X_train_raw, y_train_raw, _ = load_data_efficiently(train_dir, 'train', max_memory_gb=12)
        ...
        if os.path.exists(val_dir):
            X_val_raw, y_val_raw, _ = load_data_efficiently(val_dir, 'val', max_memory_gb=4)
            logger.info(f"Loaded {len(X_val_raw)} validation samples")
        else:
            # Créer un ensemble de validation à partir des données d'entraînement
            logger.info("Creating validation set from training data...")
        ...
        checkpoint_path = os.path.join(models_path, 'lsf_recognition_model_enhanced.keras')
        ...
        history = model.fit(
            ...
            batch_size=32,  # Batch size normal
        ...
        logger.error(f"ERROR in train_model_enhanced.py: {str(e)}")
        import traceback
        traceback.print_exc()
        raise
